...
let nested_live_continuations () =
  ...
  (* Clear the reference to make ignored continuations unreachable *)
  Gc.full_major ();
  Printf.printf "Full major GC completed. Continuations should be in the LL.\n"; flush stdout

let () =
  print_endline "Running nested live continuations example..."; flush stdout;
  nested_live_continuations ();
  print_endline "Example done."; flush stdout