...
CAMLexport void caml_gc_dispatch (void)
{
  ...
  if (trigger == caml_young_alloc_start || caml_requested_minor_gc){
    ...
    while (Caml_state->young_ptr - caml_young_alloc_start < Max_young_whsize){
...
/* Called by [Alloc_small] when [Caml_state->young_ptr] reaches
   [Caml_state->young_limit]. We have to either call memprof or the gc. */
void caml_alloc_small_dispatch (tag_t tag, intnat wosize, int track)
{
  if (Caml_state->young_ptr < caml_young_trigger){
