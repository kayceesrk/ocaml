...
let prng_key = Domain.TLS.new_key Random.State.make_self_init
...
let create ?(random = Atomic.get randomized) initial_size =
  ...
  let seed =
    if random then Random.State.bits (Domain.TLS.get prng_key) else 0
  in
...
let rebuild ?(random = Atomic.get randomized) h =
  ...
  let seed =
    if random then Random.State.bits (Domain.TLS.get prng_key)
