...
let last_search_result_key = Domain.TLS.new_key (fun () -> [||])

let string_match re s pos =
  let res = re_string_match re s pos in
  Domain.TLS.set last_search_result_key res;
  Array.length res > 0
...
let group_beginning n =
  let last_search_result = Domain.TLS.get last_search_result_key in
  let n2 = n + n in
