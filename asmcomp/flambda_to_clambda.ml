...
and to_clambda_named t env var (named : Flambda.named) : Clambda.ulambda =
  ...
  | Read_symbol_field (symbol, field) ->
    Uprim (Pfield (field, Pointer, Mutable), [to_clambda_symbol env symbol], Debuginfo.none)
  ...
  | Prim (Pfield (index, ptr, mut), [block], dbg) ->
    Uprim (Pfield (index, ptr, mut), [check_field (subst_var env block) index None], dbg)
    ...
    let env =
      ...
      let add_env_free_variable id _ env =
        ...
        Env.add_subst env id
          (Uprim (Pfield (pos, Pointer, Mutable), [Clambda.Uvar env_var], Debuginfo.none))
      in
